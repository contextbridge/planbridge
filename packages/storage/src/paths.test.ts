...
import { ensureStorageDirectory, resolveStoragePath } from './paths.ts';

describe('resolveStoragePath', () => {
  test('uses CONTEXTBRIDGE_DB_PATH override exactly', () => {
    expect(
      resolveStoragePath({
        env: { CONTEXTBRIDGE_DB_PATH: 'relative/custom.sqlite', HOME: '/home/tester' },
        platform: 'linux',
      }),
  ...
  test('uses absolute XDG_DATA_HOME on Linux', () => {
    expect(resolveStoragePath({ env: { XDG_DATA_HOME: '/data', HOME: '/home/tester' }, platform: 'linux' })).toBe(
      '/data/contextbridge/db.sqlite',
    );
  });

  test('falls back to HOME local share on Linux', () => {
    expect(resolveStoragePath({ env: { HOME: '/home/tester' }, platform: 'linux' })).toBe(
      '/home/tester/.local/share/contextbridge/db.sqlite',
    );
  });

  test('ignores relative XDG_DATA_HOME values', () => {
    expect(resolveStoragePath({ env: { XDG_DATA_HOME: 'relative', HOME: '/home/tester' }, platform: 'linux' })).toBe(
      '/home/tester/.local/share/contextbridge/db.sqlite',
    );
  });

  test('uses macOS application support', () => {
    expect(resolveStoragePath({ env: { HOME: '/Users/tester' }, platform: 'darwin' })).toBe(
  ...
  test('falls back to supplied homedir when HOME is unavailable', () => {
    expect(resolveStoragePath({ env: {}, platform: 'linux', homedir: () => '/fallback' })).toBe(
