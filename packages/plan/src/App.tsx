...
export function App({ initialPayload, initialThreads, initialGlobalComment }: AppProps = {}) {
  const { fetchPayload, fetchUpdateNotice, analytics, buildInfo } = usePlanAppContext();
  const [payload, setPayload] = useState<SubmissionPayload | null>(initialPayload ?? null);
  const [updateNotice, setUpdateNotice] = useState<UpdateNotice | null>(null);
  const [updateNoticeDismissed, setUpdateNoticeDismissed] = useState(false);
  const reviewState = usePlanReviewState({ initialThreads, initialGlobalComment });
  ...
  useEffect(() => {
    ...
    fetchPayload()
      ...
      .catch(() => setPayload({ content: '(unable to load plan)' }));
