...
export interface UsePlanReviewStateArgs {
...
export function usePlanReviewState({ initialThreads, initialGlobalComment }: UsePlanReviewStateArgs = {}) {
  const { submitPlanReview, scheduleTimeout, closeWindow, autoCloseDelaySeconds } = usePlanAppContext();
  ...
  const submitStatus: PlanReviewStatus = feedbackCount === 0 ? 'approved' : 'changes_requested';
  ...
  const submit = async () => {
    ...
    const submission: PlanReviewSubmission = {
      status: submitStatus,
    ...
    try {
      await submitPlanReview(submission);
      closeDraft();
      setSubmitted(true);
