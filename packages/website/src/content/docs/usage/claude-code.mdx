...
The plan-mode hook fires automatically. To pull an arbitrary markdown file into the PlanBridge UI on demand (a design doc, an RFC, a plan written outside of plan mode), use the `/planbridge-open` command that ships with the plugin. See [Open arbitrary content](/usage/open/).
