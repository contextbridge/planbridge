...
This is the manual sibling to the automatic hook flow. Hooks fire when your agent finishes planning. The manual open action fires whenever you ask for it.
...
The `planbridge-open` command ships with the `planbridge@contextbridge` Claude plugin. Invoke it inside Claude Code with:
...
The manual command and skill are thin wrappers around `contextbridge open`. You can run the binary yourself from any harness:
