...
If you have our Codex CLI hook installed (via `contextbridge install`), PlanBridge opens whenever Codex finishes a turn with a proposed plan. You review the plan in the browser, and your decision flows back through Codex's `Stop` hook.
...
Or `contextbridge uninstall` to remove PlanBridge wiring from every harness it was installed into.

## Open a file manually

The `Stop` hook fires automatically. To pull an arbitrary markdown file into the PlanBridge UI on demand, use the `$planbridge-open` skill that `contextbridge install codex` writes to `~/.agents/skills/planbridge-open/`. See [Open arbitrary content](/usage/open/).
