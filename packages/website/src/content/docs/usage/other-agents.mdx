...
```markdown
Whenever you produce a new plan, pipe it to `contextbridge plan` on stdin and read
the markdown returned on stdout. The response will tell you whether your plan
was approved or needs revision and what to change. If changes are requested,
submit the next version with the returned `contextbridge plan --plan-id <id>`
command so it is recorded as a revision of the same plan. Follow the
instructions in the response. If the command exits with a non-zero status,
stop.
```
...
- `# Plan review: changes requested`. The human left feedback. Revise the plan to address every comment, then run the returned `contextbridge plan --plan-id <id>` command with the revised plan. Do not start over with a plain `contextbridge plan` unless the next plan is genuinely different.
